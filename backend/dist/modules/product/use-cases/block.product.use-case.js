...
let BlockProductUseCase = class BlockProductUseCase {
    _productRepository;
    ...
    async execute(id) {
        ...
        if (!product.isActive) {
            throw new CustomError(ResponseMessages.PRODUCT_ALREADY_INACTIVE, HttpStatusCode.BAD_REQUEST);
        }
        const deactivated = product.deactivate();
        await this._productRepository.save(deactivated);
    }
};
BlockProductUseCase = __decorate([
    injectable(),
    __param(0, inject("IProductRepository")),
    __metadata("design:paramtypes", [Object])
], BlockProductUseCase);
export { BlockProductUseCase };
//# sourceMappingURL=block.product.use-case.js.map