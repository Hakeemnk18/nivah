...
export class CartMapper {
    /* ================= DOMAIN ================= */
    static toDomain(cartModelData) {
        if (!cartModelData)
            return null;
        const idString = cartModelData._id?.toString() || cartModelData.id?.toString();
        if (!idString) {
        ...
        return new Cart({
            ...
            items: cartModelData.items?.map((item) => ({
    ...
    /* ================= PERSISTENCE ================= */
    static toPersistence(cartEntity) {
        return {
