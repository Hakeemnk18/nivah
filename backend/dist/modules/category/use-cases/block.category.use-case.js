...
let BlockCategoryUseCase = class BlockCategoryUseCase {
    _categoryRepository;
    ...
    async execute(id) {
        ...
        if (!category.isActive) {
            throw new CustomError(ResponseMessages.CATEGORY_ALREADY_INACTIVE, HttpStatusCode.BAD_REQUEST);
        }
        const deactivated = category.deactivate();
        await this._categoryRepository.save(deactivated);
    }
};
BlockCategoryUseCase = __decorate([
    injectable(),
    __param(0, inject("ICategoryRepository")),
    __metadata("design:paramtypes", [Object])
], BlockCategoryUseCase);
export { BlockCategoryUseCase };
//# sourceMappingURL=block.category.use-case.js.map