...
let BlockTestimonialUseCase = class BlockTestimonialUseCase {
    ...
    async execute(id) {
        ...
        if (!testimonial.isActive) {
            throw new CustomError(ResponseMessages.TESTIMONIAL_ALREADY_DEACTIVATED, HttpStatusCode.BAD_REQUEST);
        }
        const updatedTestimonial = testimonial.deactivate();
        const saved = await this._testimonialRepository.save(updatedTestimonial);
        if (!saved) {
            throw new CustomError(ResponseMessages.TESTIMONIAL_DEACTIVATE_FAILED, HttpStatusCode.INTERNAL_SERVER_ERROR);
        }
    }
};
BlockTestimonialUseCase = __decorate([
    injectable(),
    __param(0, inject("ITestimonialRepository")),
    __metadata("design:paramtypes", [Object])
], BlockTestimonialUseCase);
export { BlockTestimonialUseCase };
//# sourceMappingURL=block.testimonial.use-case.js.map