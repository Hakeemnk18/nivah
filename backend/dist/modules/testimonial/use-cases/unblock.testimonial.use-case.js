...
let UnblockTestimonialUseCase = class UnblockTestimonialUseCase {
    ...
    async execute(id) {
        ...
        if (testimonial.isActive) {
            throw new CustomError(ResponseMessages.TESTIMONIAL_ALREADY_ACTIVATED, HttpStatusCode.BAD_REQUEST);
        }
        const updatedTestimonial = testimonial.activate();
        const saved = await this._testimonialRepository.save(updatedTestimonial);
        if (!saved) {
            throw new CustomError(ResponseMessages.TESTIMONIAL_ACTIVATE_FAILED, HttpStatusCode.INTERNAL_SERVER_ERROR);
        }
    }
};
UnblockTestimonialUseCase = __decorate([
    injectable(),
    __param(0, inject("ITestimonialRepository")),
    __metadata("design:paramtypes", [Object])
], UnblockTestimonialUseCase);
export { UnblockTestimonialUseCase };
//# sourceMappingURL=unblock.testimonial.use-case.js.map