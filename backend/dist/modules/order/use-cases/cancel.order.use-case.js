...
let CancelOrderUseCase = class CancelOrderUseCase {
    ...
    async execute(orderId) {
        ...
        if (order.orderStatus === "delivered") {
            throw new CustomError(ResponseMessages.ORDER_DELIVERED, HttpStatusCode.BAD_REQUEST);
        }
        if (order.orderStatus === "cancelled") {
            throw new CustomError(ResponseMessages.ORDER_CANCELLED, HttpStatusCode.BAD_REQUEST);
        }
        await this._orderRepository.cancelOrder(orderId);
    }
};
CancelOrderUseCase = __decorate([
    injectable(),
